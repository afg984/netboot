...
func placeholderFunc(s string) func() string {
	return func() string {
		return s
	}
}
...
func StaticBooter(spec *Spec) (Booter, error) {
	...
	cmdline, err := expandCmdline(spec.Cmdline, template.FuncMap{"ID": f, "MACAddress": placeholderFunc("MACAddress")})
	if err != nil {
		return nil, err
	}
...
func (b *apibooter) BootSpec(m Machine) (*Spec, error) {
	...
	ret.Cmdline, err = expandCmdline(ret.Cmdline, template.FuncMap{"URL": f, "MACAddress": placeholderFunc("MACAddress")})
	if err != nil {
		return nil, err
	}
