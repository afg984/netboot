...
func archlinuxRecipe(parent *cobra.Command) {
	archCmd := &cobra.Command{
		Use:   "archlinux [version]",
...
version defaults to latest, can also be a YYYY.MM.DD iso release version`,
		Run: func(cmd *cobra.Command, args []string) {
			...
			cmdline := fmt.Sprintf("archisobasedir=arch archiso_http_srv=%s/ ip=dhcp verify=y net.ifnames=0 BOOTIF={{ MACAddress }}", httpSrv)
